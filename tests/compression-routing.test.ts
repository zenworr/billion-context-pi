    ...
    modelRegistry: {
      ...
      getProvider: () => ({
        streamSimple: (
          ...
          result: async () => {
            const response = await complete(selected, `${context.systemPrompt ?? ""}\n${context.messages[0]!.content[0]!.text}`, options.reasoning);
...
test("configured Tier-1 compressor generates and anchors the summary", async (t) => {
  ...
  assert.equal(state.blocks[0].summary, "The source established port 4317 and required preserving src/telemetry.ts as a durable implementation constraint.");
  ...
  assert.equal(serializedTurn.match(/The source established port 4317/g)?.length, 1);
...
test("tier promotion hides inactive generated child anchors", async (t) => {
  ...
  assert.equal(serializedTurn.match(/PARENT_DISTILLED/g)?.length, 1);
});

...
import { compressionThinkingLevel, compressorModeForTier, effectiveActiveTarget, parseCompressionModel, resolveConfig, resolveDelegate, safeResumeThreshold, type AdapterConfig } from "../src/config.js";

const EMPTY: AdapterConfig = {};

test("budget controls active target, hard threshold, emergency threshold, and safe resume", () => {
  const adapter: AdapterConfig = {
    budget: {
      targetActiveTokens: 150_000,
      targetContextPercent: 0.30,
      hardContextPercent: 0.75,
      emergencyContextPercent: 0.92,
      outputReserveTokens: 16_000,
      safetyMarginTokens: 8_000,
    },
  };
  const config = resolveConfig(adapter, 500_000);
  assert.equal(effectiveActiveTarget(adapter, 500_000), 150_000);
  assert.equal(safeResumeThreshold(adapter, 500_000), 166_000);
  assert.equal(config.nudge.maxContextLimitPct, 0.75);
  assert.equal(config.nudge.emergencyThresholdPct, 0.92);
  assert.equal(config.truncate.threshold, 0.92);
});
...
test("resolveConfig supplies and merges deterministic clearing policy", () => {
  const defaults = resolveConfig(EMPTY, 200_000).clearing;
  assert.equal(defaults.enabled, true);
  assert.equal(defaults.keepRecentToolUses, 5);
  assert.equal(defaults.clearAtLeastTokens, 16_000);
  assert.equal(defaults.clearToolInputs, false);
  ...
  const configured = resolveConfig({
    clearing: {
      ...
      excludeTools: ["custom_secret_tool"],
    },
  }, 200_000).clearing;
  assert.equal(configured.keepRecentToolUses, 8);
  assert.equal(configured.clearAtLeastTokens, 20_000);
  assert.deepEqual(configured.excludeTools, ["compress", "edit", "write", "memory_write", "custom_secret_tool"]);
});
...
test("Phase-7 optimization behavior is safe and off unless configured", () => {
  const adapter: AdapterConfig = {};
  assert.notEqual(adapter.optimization?.automaticDistillation, true);
  assert.notEqual(adapter.optimization?.shadowCompaction, true);
  assert.notEqual(adapter.optimization?.costAwareRouting, true);
  assert.notEqual(adapter.optimization?.telemetry, true);
  assert.notEqual(adapter.optimization?.qualityAdaptation, true);
});

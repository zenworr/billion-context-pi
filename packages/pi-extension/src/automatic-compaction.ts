...
export function wireAutomaticCompaction(pi: ExtensionAPI, runtime: AcpRuntime): AutomaticCompactionController {
  ...
  pi.on("turn_end", (event, ctx) => {
    ...
    controller.schedule("turn_end", ctx);
  });
...
async function captureSnapshot(runtime: AcpRuntime, ctx: ExtensionContext, signal: AbortSignal): Promise<AutomaticSnapshot | undefined> {
  ...
  try {
    ...
    const candidate = selectCandidate(state.blocks, minimumBlocks, minimumTokens, runtime);
    ...
    return {
      ...
      sourceHashes,
      treeKey: entries.at(-1)?.id ?? "empty",
      modelKey: activeModelKey(ctx),
      tier: candidate.tier,
...
function selectCandidate(
  ...
  for (const tier of [2, 3] as const) {
    ...
    const candidates = blocks.filter((block) => block.active && block.tier === sourceTier);
    const tokens = candidates.reduce((sum, block) => sum + Math.max(1, Math.ceil(block.summary.length / 4)), 0);
    if (candidates.length >= minimumBlocks && tokens >= minimumTokens) return { tier, sourceTier, blocks: candidates };
  }
  return undefined;
}
